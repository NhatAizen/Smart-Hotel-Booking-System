...
import com.smarthotel.payment.wallet.service.RoleChangePaymentEligibilityService;
...
public class RoleChangePaymentController {

    private final RoleChangePaymentEligibilityService eligibilityService;

    public RoleChangePaymentController(
            RoleChangePaymentEligibilityService eligibilityService
    ) {
        this.eligibilityService = eligibilityService;
    }

    @GetMapping("/payment-eligibility")
    public ResponseEntity<RoleChangePaymentEligibilityResponse> getPaymentEligibility(
            @AuthenticationPrincipal Jwt jwt,
            @PathVariable UUID ownerId
    ) {
        ensureCanInspectOwner(jwt, ownerId);
        return ResponseEntity.ok(eligibilityService.getEligibility(ownerId));
    }
    ...
    private void ensureCanInspectOwner(Jwt jwt, UUID ownerId) {
        ...
        throw new AccessDeniedException(
                "Bạn không có quyền kiểm tra tài chính của tài khoản này"
        );
    }
